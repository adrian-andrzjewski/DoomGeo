...
| Area | Status |
| --- | --- |
| WAD conversion | Converts E1M1 map lumps, player start, doors, exits, secrets, damaging sectors, texture classes, sector heights, and runtime things into a centered `48x36` Neo Geo grid. The default conversion now favors a deliberately lower-complexity collision map instead of preserving every WAD line as a full-height raycast obstacle. Generated map arrays are split into `doom_map_generated.h` declarations and `doom_map_generated.c` table definitions. |
