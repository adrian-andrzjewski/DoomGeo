...
| Area | Status |
| --- | --- |
| WAD conversion | Converts E1M1 map lumps, player start, doors, exits, secrets, damaging sectors, texture classes, sector heights, and runtime things into a centered `48x36` Neo Geo grid for the normal experimental path. `DOOM_SIMPLE_MAP=1` can also stream WAD-derived `16x16` chunks generated by `tools/doom_chunk_convert.py`; the default chunk scale is 64 Doom units per cell, while the current manual chunk/RIPDOOM-lite build uses 32 Doom units per cell through `make chunk-playable-rom`. The renderer still loads only one `16x16` page at a time. Generated map arrays are split into `doom_map_generated.h` declarations and `doom_map_generated.c` table definitions. |
...
For the current playable E1M1 chunk/RIPDOOM-lite path, use:
