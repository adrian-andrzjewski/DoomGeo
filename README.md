...
| Area | Status |
| --- | --- |
| WAD conversion | Converts E1M1 map lumps, player start, doors, exits, secrets, damaging sectors, texture classes, sector heights, and runtime things into a centered `48x36` Neo Geo grid for the normal experimental path. `DOOM_SIMPLE_MAP=1` instead asks the converter for a `16x16` support grid while gameplay uses an authored active `16x16` map, which is the current baseline for the planned chunked WAD simplifier. Generated map arrays are split into `doom_map_generated.h` declarations and `doom_map_generated.c` table definitions. |
...
make cart DOOM_DETAIL=clarity     # 64 wall columns, one visible world thing
...
make route-check
make DOOM_MAP=E1M1 BUILDDIR=build/chunk-e1m1 chunk-map chunk-route-check
