...
GFX_TITLEPIC_ARG=$(if $(filter 0 no false,$(DOOM_TITLEPIC)),--no-titlepic,)
...
ifeq ($(DOOM_TITLEPIC),0)
override CFLAGS += -DDOOM_TITLEPIC=0
endif
...
ifneq ($(strip $(DOOM_PLANE_FLOOR_FORWARD_PHASES)),)
override CFLAGS += -DDOOM_PLANE_FLOOR_FORWARD_PHASES=$(DOOM_PLANE_FLOOR_FORWARD_PHASES)
endif
...
$(GFX_STAMP): tools/gen_gfx.py tools/doom_convert.py config.h $(DOOM_MAP_HEADER) $(DOOM_IWAD) | $(BUILDDIR) $(GFX_ROM_DIR)
	$(PYTHON) tools/gen_gfx.py --iwad $(DOOM_IWAD) --map $(DOOM_MAP) --wall-texture $(DOOM_WALL_TEXTURE) --detail $(DOOM_DETAIL) $(GFX_SIMPLE_MAP_ARG) $(GFX_PLANE_PHASE_ARGS) $(GFX_CEILING_ARG) $(GFX_TITLEPIC_ARG) $(GFX_CROM_BYTES_ARG) $(GFX_MONSTER_SPRITES_ARG) $(GFX_SPRITE_SCALES_ARG) --palette-header $(GFX_HEADER) --out-dir $(GFX_ROM_DIR)
	touch $@
