...
be played directly from the repository's Pages site.
...
(`doomgeo.zip`) for download links, but those are not the files passed to the
emulator. The raw build artifacts under `build/rom/` are not rewritten this
way; this compatibility package is only for the browser player.
